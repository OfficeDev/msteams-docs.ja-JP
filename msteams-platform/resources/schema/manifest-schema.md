...
|`needsChannelSelector`|boolean|||ボットがユーザー ヒントを使用して特定のチャネルにボットを追加するかどうかを説明します。 既定値: **`false`**|
...
|`parameters.name`|文字列|64 文字|✔|クライアントに表示されるパラメーターの名前。 パラメーター名は、ユーザー要求に含まれます。|
