...
1. プラグインがインストールされたら、**サイト管理者** ページの [**プラグイン**] タブに移動し、[**認証**] セクションリンクを選択して **OpenID Connect** を有効にします。 プラグインの構成を空白のままにしても問題ありません。後で入力されます。
...
Moodle コースが同期されると、Teams はチームに自動的に Moodle アプリをインストールし、Teams の [全般] チャネルに [Moodle] タブを作成し、同期元の Moodle コースのコース ページを含む構成を行います。 これで、Teams から直接 Moodle コースの操作を開始できます。
