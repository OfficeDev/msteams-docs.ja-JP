...
>
> * Moodle サーバーの自動登録 (Microsoft Azure Active Directory [(Microsoft Azure Active Directory Azure AD ))を使用します](https://azure.microsoft.com/services/active-directory/)。
>
...
* Microsoft Azure Active Directory (Azure AD) 管理者資格情報。
...
## <a name="2-configure-the-connection-between-the-microsoft-365-plugins-and-microsoft-azure-active-directory-microsoft-azure-active-directory-azure-ad"></a>2. Microsoft 365 プラグインと Microsoft Azure Active Directory (Microsoft Azure Active Directory (Azure AD)) の間の接続を構成します。

アプリケーション プラグインとアプリケーション (Microsoft 365) の間Microsoft Azure Active Directory構成Azure AD。

### <a name="requisites"></a>前提条件

PowerShell スクリプトを使用して、Microsoft Azure Active Directory (Azure AD) で Moodle をアプリケーションとして登録します。 スクリプトは、次の情報を準備します。

* 新しいMicrosoft Azure Active Directory (Azure AD) アプリケーションは、Microsoft 365 Microsoft 365 Moodle プラグインによって使用されます。
* テナントのアプリMicrosoft 365、`AppID`プロビジョニングされたアプリに必要な返信 URL とアクセス許可を設定し、およびを返します`Key`。

Moodle サーバー サイト`AppID``Key`を構成するには、Microsoft 365の [Moodle プラグイン] セットアップ ページで生成されたページを使用Microsoft Azure Active Directory (Azure AD)。
...
1. PowerShell スクリプトを実行します。
    ...
    1. Moodle プラグインや Moodle プラグインMicrosoft Azure Active Directory (Azure AD) アプリケーションの名前を入力します。
...
1. ページが更新された後、別の新しいセクション [管理者の同意] が表示 **され、&が表示されます**。
    ...
    1. [テナント] Microsoft Azure Active Directory **(Azure AD) フィールドの** 横にある [検出] ボタン **を選択** します。
...
1. Moodle サーバーとユーザーの間でユーザーを同期Microsoft Azure Active Directory (Azure AD)。 次の手順をお試しください。

    > [!NOTE]
    > 環境に応じて、この段階でさまざまなオプションを選択できます。

1. Moodle サーバーとユーザーの間でユーザーを同期Microsoft Azure Active Directory (Azure AD)。 環境に応じて、この段階でさまざまなオプションを選択できます。 次の手順をお試しください。
    1. [同期] タブ **に設定します**。

    1. [ユーザー **とユーザーを同期Microsoft Azure Active Directory (Azure AD)** セクションで、環境に適用するチェック ボックスをオンにします。 次の項目を選択する必要があります。  

        ✔ [アカウントを作成する] ( [アカウント] ) の [Microsoft Azure Active Directory] (Azure AD)。

        ✔ Moodle のすべてのアカウントを更新します (Microsoft Azure Active Directoryユーザー Azure AD)。

    1. [ユーザー **作成の制限]** セクションで、Moodle に同期Microsoft Azure Active Directory (Azure AD) ユーザーを制限するフィルターをセットアップできます。
    1. [**ユーザー フィールド マッピング]** セクションを使用すると、Microsoft Azure Active Directory (Azure AD) を Moodle ユーザー プロファイル フィールド マッピングにカスタマイズできます。
    1. [同期 **Teams]** セクションで、既存の Moodle コースの一部またはすべてのチームなどのグループを自動的に作成する場合に選択できます。

13. cron ジョブ [を検証 Microsoft Azure Active Directoryし](https://docs.moodle.org/310/en/Cron)、最初の実行に対して手動で実行するには、[ユーザーと Azure AD を同期する] セクションの [スケジュール **されたタスク** の管理] ページ リンクを選択します。 これにより、[スケジュールされたタスク] **ページに移動** します。

    1. 下にスクロールして、[ユーザーと同期] (Microsoft Azure Active Directory **) ジョブを** Azure ADし、[今すぐ実行] **を選択します**。
