...
description: ユーザー向け Moodle 統合アプリをインストールして構成するMicrosoft Teams
...
> [!NOTE]
> IT 管理者が簡単に Moodle と Teams統合をセットアップするために、オープン ソース Microsoft 365 Moodle Plugins は次の情報に対して更新されます。
>
> * Moodle サーバーの自動登録 (Microsoft Azure Active Directory[)をAzure AD](https://azure.microsoft.com/services/active-directory/)。
>
...
Moodle の統合はMicrosoft Teams、Moodle プラグインセットMicrosoft 365[によって実行されます](https://github.com/Microsoft/o365-moodle)。
...
1. Moodle [OpenID](https://moodle.org/plugins/auth_oidc) プラグインとConnect統合[Microsoft 365](https://moodle.org/plugins/local_o365)をローカル コンピューターにダウンロードして保存します。
    ...
    > [!NOTE]
    > 統合に必要な OpenID ConnectおよびMicrosoft 365統合プラグインのインストールTeamsです。
    >
...
1. 管理者として Moodle サーバーにサインインし、左側のナビゲーション  パネルにある [設定[](https://docs.moodle.org/22/en/Settings_block)] ブロックから [サイトの管理] を選択します。

1. [プラグイン] **タブを選択** し、[プラグインのインストール **] を選択します**。

1. [ZIP ファイル **からプラグインをインストールする] セクションで、[** ファイルの **選択] を選択します**。

1. 左側 **アップロードから [** ファイル] オプションを選択し、ダウンロードしたファイルを参照して、この **アップロード選択します**。
...
## <a name="2-configure-the-connection-between-the-microsoft-365-plugins-and-azure-ad"></a>2. プラグインとアプリケーション間の接続Microsoft 365構成Azure AD

アプリケーション プラグインとアプリケーション プラグイン間のMicrosoft 365構成するAzure AD。
...
* テナントのアプリMicrosoft 365、プロビジョニングされたアプリに必要な返信 URL とアクセス許可を設定し、およびを返`AppID`します`Key`。

Moodle サーバー サイト`AppID`を`Key`構成するには、Microsoft 365の [Moodle プラグイン] セットアップ ページで生成され、Azure AD。
...
1. 次に、Moodle `AppID` `Key` プラグインを追加し、Microsoft 365する必要があります。 プラグインの管理ページ、サイトの管理、プラグイン>統合> Microsoft 365します。
...
1. ページが更新された後、別の新しいセクション [管理者の同意] & **を表示できます**。
    1. [**管理者の同意の提供**] リンクを選択し、グローバル管理者Microsoft 365資格情報を入力し、[**承諾**] を選択してアクセス許可を付与します。
...
1. Moodle サーバーとサーバー間でユーザーを同期Azure AD。 環境に応じて、この段階でさまざまなオプションを選択できます。 次の手順をお試しください。
    ...
    1. [ユーザー **とユーザーを同期Azure AD**] セクションで、環境に適用するチェック ボックスをオンにします。 次の項目を選択する必要があります。  
        ...
        ✔ アカウント内のユーザーの Moodle のすべてのアカウントを更新Azure AD。

    1. [ユーザー **作成の制限] セクション** で、Moodle に同期Azure ADを制限するフィルターを設定できます。
    1. [**ユーザー フィールド マッピング]** セクションでは、Moodle ユーザー プロファイル Azure ADマッピングの設定をカスタマイズできます。
    1. [同期 **Teams]** セクションで、既存の Moodle コースの一部またはすべてのチームなどのグループを自動的に作成する場合に選択できます。

13. cron ジョブ [を検証し](https://docs.moodle.org/310/en/Cron)、最初の実行に対して手動で実行するには、[ユーザーとタスクを同期する] セクションの [スケジュールされたタスク **の管理] Azure AD** します。 これにより、[スケジュールされたタスク] **ページに移動** します。

    1. 下にスクロールして、[ユーザーと同期 **] ジョブをAzure AD** を選択 **します**。
    1. 既存のコースに基づいてグループを作成する場合は、ユーザー グループの作成ジョブでユーザー **グループMicrosoft 365** することもできます。
...
1. [プラグインの管理] ページ、[**サイトの** 管理] >統合> Microsoft 365に戻り、[プラグイン] ページ **Teams 設定します。**

1. [アプリの **Teams 設定**] ページで、アプリの統合を有効にTeams設定を構成します。
    ...
    1. 外部サービスを有効にするには、[Microsoft 365サービス] **リンクを選択** し、次の項目を選択します。  
...
無料の Moodle アシスタント ボットはMicrosoft Teams教師と学生が、Moodle のコース、課題、成績、その他の情報に関する質問に答えるのに役立ちます。 ボットは、このボット内の生徒や教師に、Moodle 通知Teams。 ボットは、Microsoft が管理するオープン ソース プロジェクトであり、そのプロジェクトで[GitHub](https://github.com/microsoft/Moodle-Teams-Bot)。
...
ボットを登録するには、次の方法を使用します。

1. [プラグインの管理] ページに移動し、[プラグイン] **を選択します**。 [統合 **Microsoft 365] の** 下で、[統合] **タブTeams 設定** します。

1. [ **Microsoft アプリケーション登録ポータル] リンクを選択** し、Microsoft ID でサインインします。

1. アプリの名前 (MoodleBot など) を入力し、[作成] ボタン **を選択** します。

1. アプリケーション ID **をコピーし**、[チーム] ページの [**ボット アプリケーション ID****] フィールド設定** します。
...
> [!div class="checklist"]
>
> * [**Azure に展開する**] を選択し、ボット アプリケーション ID、ボット アプリケーション パスワード、および [アカウント] ページの [Moodle シークレット] など、必要な情報を含むフォーム **Teams 設定** します。 Azure の情報は、[セットアップ] ページ **に表示** されます。
...
ボットが Azure に展開され、Moodle サーバーと話をするように構成した後、アプリを展開するMicrosoft Teamsがあります。 これを行うには、前の手順の [Moodle Plugins Team Microsoft 365] ページからダウンロード設定アプリ マニフェスト ファイルを読み込む必要があります。
...
4. 以前にダウンロード `manifest.zip` したパッケージを選択し、[保存] を **選択します**。 アプリ マニフェスト パッケージをダウンロードしていない場合は、Moodle の [プラグイン構成] ページ設定 [チーム] タブからダウンロードできます。
...
Moodle タブの自動作成を許可するには、次の方法を実行します。

1. Microsoft Teams を開きます。

1. ナビゲーション バーの左下の領域から [アプリ] アイコンを選択します。

1. アップロードされた **Moodle アプリを見つけて>** オプション アイコン **を選択** し>コピー リンク **を選択します**。
...
1. [Moodle] で、[ムー Teamsプラグインの構成] ページから [Microsoft 365] [ムードル アプリ] タブを開きます。

1. アプリの ID を [Moodle Microsoft Teams ID] フィールドに貼り付けて、変更を保存します。
