...
---
# <a name="together-mode-in-teams"></a>[一緒に] モード (Teams

> [!NOTE]
> この機能は現在、パブリック開発者 [プレビューでのみ利用](../resources/dev-preview/developer-preview-intro.md) できます。

Microsoft TeamsTogether Mode は、没入感のある魅力的な会議環境を提供し、ユーザーを集め、ビデオを有効にできます。 参加者を 1 つの仮想シーンにデジタル的に組み合わせ、シーン作成者によって設計および固定された事前に決定されたシートにビデオ ストリームを設定します。

> [!VIDEO https://www.youtube-nocookie.com/embed/MGsNmYKgeTA]

Together Mode のシーンは、Microsoft Scene studio を使用してシーン開発者によって作成されたアーティファクトです。 考え出されたシーンの設定では、参加者は指定されたシートを持ち、それらのシートにビデオ ストリームがレンダリングされます。
...
Together Mode を使用するには、以下の基本的な理解が必要です。
...
1. 表示されるダイアログ ボックスで、[追加] を **選択します**。
    ...
    テスト会議を作成し、一緒にモードを起動することで、シーンをテストまたはアクセスできます。 詳細については、「一緒にモード [をアクティブにする」を参照してください](#activate-the-together-mode)。
    ...
    > * その後、シーンを Together Mode シーン ギャラリーで表示できます。
...
Scene.jsと共にオンに設定すると、シートの正確な位置が示されます。 シーンは、参加者のビデオを入れるビットマップ イメージ、スプライト、長方形で構成されます。 これらのスプライトと参加者ボックスはワールド座標系で定義され、X 軸は右を指し、Y 軸は下向きを指します。 一緒にモードでは、現在の参加者の拡大がサポートされます。 これは、大規模なシーンでの小規模な会議に役立ちます。 スプライトは、世界に配置された静的なビットマップ イメージです。 スプライトの Z 値は、スプライトの位置を決定します。 レンダリングは、Z 値が最も低いスプライトから始まるので、Z 値が高いほどカメラに近くなります。 各参加者は独自のビデオ フィードを持ち、フォアグラウンドのみをレンダリングするためにセグメント化されます。
...
サンプル をオンにしたので、scene.jsモードをアクティブにし、シーンに参加できます。

## <a name="activate-the-together-mode"></a>一緒にモードをアクティブ化する

エンド ユーザーが Together モードでシーンとどのように関わるかについて、エンドツーエンドの情報を取得します。

**シーンを選択し、一緒にモードをアクティブにするには**
...
1. 必要に応じて、会議の開催者と発表者は、会議で [すべての参加者を一緒にモードに切り替 **える]** を選択できます。

    >[!NOTE]
    > 任意の時点で、会議に同種のシーンを 1 つしか使用できません。 発表者または開催者がシーンを変更すると、すべてのシーンが変更されます。 Together モードの切り替えまたは切り替えは個々の参加者に対して行いますが、Together Mode では、すべての参加者が同じシーンを持っています。

1. **[適用]** を選択します。 Teamsアプリをインストールし、シーンを適用します。

## <a name="open-a-together-mode-scene-package"></a>一緒にモードのシーン パッケージを開く
