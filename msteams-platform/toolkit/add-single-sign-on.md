...
1. コピー`auth-start.html`して`auth-end.htm`\*\*フォルダー内に`auth/public`移動します。`tabs/public/` Teams Toolkit は、Azure AD のリダイレクト フローに対して、これら 2 つのエンドポイントを Azure AD に登録します。
...
2. [コピー先] の下にある`auth/tab`フォルダーをコピー`sso`します`tabs/src/sso/`。
   ...
   * `GetUserProfile`: このファイルは、Microsoft Graph APIを呼び出してユーザー情報を取得する関数を実装します。
...
1. フォルダー内で `npm install isomorphic-fetch --save` コマンドを `bot` 実行します。
1. ts スクリプトの場合は、フォルダー内のコマンドを `npm install copyfiles --save-dev` 実行し、 `bot` 次の行を次の行に `package.json`置き換えます。
...
<details>
<summary><b>メッセージ拡張プロジェクト </b></summary>

サンプル ビジネス ロジックは、TeamsActivityHandler を拡張してオーバーライドするハンドラー `TeamsBot` を提供します `handleTeamsMessagingExtensionQuery`。

ログインしている Teams ユーザー トークンを `handleMessageExtensionQueryWithToken` 使用して取得される、with トークン内のクエリ ロジックを更新できます。

アプリケーションでこれを機能させるには、次の手順を実行します。

1. フォルダーを移動します`auth/bot/public``bot`。 このフォルダーには、ボット アプリケーションがホストする HTML ページが含まれています。 Azure AD でシングル サインオン フローが開始されると、Azure AD によってユーザーがこれらのページにリダイレクトされます。

1. `bot/index`これらのページに適切な`restify`ルートを追加するように変更します。
...
1. 下のインターフェイスを`bot/teamsBot`オーバーライドします`handleTeamsMessagingExtensionQuery`。 サンプル コードに従って、独自の `handleMessageExtensionQueryWithToken` クエリ ロジックを実行できます。

1. 開き`bot/package.json`、バージョン >= 1.2.0 であることを確認します`@microsoft/teamsfx`

1. ボット プロジェクトに npm パッケージをインストール `isomorphic-fetch` します。

1. (ts のみ)ボット プロジェクトに npm パッケージをインストール`copyfiles`し、次のようにスクリプト`bot/package.json`を`build`追加または更新します。

    ```json
    "build": "tsc --build && copyfiles ./public/*.html lib/",
    ```

    これにより、このボット プロジェクトをビルドするときに、認証リダイレクトに使用される HTML ページがコピーされます。

1. で`handleMessageExtensionQueryWithToken`使用したスコープを更新`templates/appPackage/aad.template.json`します。
