...
* **機能とエクスペリエンスの詳細なテスト**
  ...
  アプリは検証コントロールによって徹底的にレビューされ、機能とユーザー エクスペリエンスの詳細なテスト、ユーザビリティ チェック、メタデータ チェックに重点を置いて、[Microsoft コマーシャル マーケットプレースの認定ポリシー](/legal/marketplace/certification-policies)に準拠していることを確認します。 アプリの検証は、デスクトップ、Web、およびモバイル クライアント間で実行されます。
...
* **アプリの申請の前に**
  ...
  1. アプリの申請の準備をするときは、[チェックリストに従い](/microsoftteams/platform/concepts/deploy-and-publish/appsource/prepare/submission-checklist)、提出パッケージの一部として次の詳細を含めます。
      ...
      1. アプリのデモ ビデオ録画キー ユーザー フローへのリンク。 これを強くお勧めします。
...
* **アプリの申請を投稿する**
  ...
  * 検証プロセス中にアプリの機能を変更しないでください。 これにより、新しい問題が発見され、アプリの承認にかかる時間が長くなる可能性があります。
