...
* コアユーザーのシナリオでは、アプリがユーザーのチームの外になることはありません。 Teams 内のユーザーに情報を表示するには、タスクモジュール/タブを使用することをお勧めします。
...
* ユーザーに対して明示的なサインインオプションが提供されている場合は、対応するサインアウトオプションがある必要があります (アプリが [サイレント認証](../../../../tabs/how-to/authentication/auth-silent-aad.md)を使用している場合でも)。
...
* アプリがアカウントまたはサービスを必要とする場合、ユーザーがサインアップするか、サインアップ要求を作成するための手段を提供する必要があります。 アプリで使用するライセンスが必要な場合は、例外が許可されることがあります。 しかし、このようなシナリオでは、新しいユーザーサインアップのための明確な転送を行う必要があります。
* アプリサービスを使用するためにサインアップする方法について、新しいユーザーに明確な転送ガイダンスを提供するようにしてください。 準備完了のサインアップリンクが利用できない場合は、次の領域で、わかりやすい方法が提供されることがあります。
...
> * アプリのウェルカムメッセージ
> * アプリのヘルプメッセージ
> * を使用して、ユーザーにサービスへのサインインを求めるウィンドウが表示されます。

* 簡単なサインアップフローを持たないアプリには、新しいユーザーが Microsoft Teams を使用してアプリを構成する方法についての詳細なガイダンスを参照できる [ヘルプ] タブまたは web ページへのリンクも含まれていることがあります。  これにより、アプリを初めて試行したときに、新しいユーザーがブロックされないようにすることができます。
...
### <a name="9989-tab-content-must-not-have-excessive-chrome-or-layered-navigation"></a>&#9989; タブのコンテンツには、chrome またはレイヤードナビゲーションが多すぎないようにする必要があります。
...
* アプリ内で複雑な編集機能を持つチャネルタブでは、タブではなく複数のウィンドウでエディタービューを開く必要があります。
...
* [ *ヘルプ* ] タブを静的タブとして追加して、アプリを構成、サインアップ、および使用する方法をユーザーにアドバイスします。
* アプリヘッダーから利用できる *設定* タブを含めてください。
...
* **チーム、グループチャット、または1:1 の会話が含ま** れます。 Bot の通知には、チーム、グループチャット、または対象ユーザーに関連するコンテンツと1対1の会話を含める必要があります。
...
* タブ、アダプティブカード、ボットメッセージ、タスクモジュール内のコンテンツは、さまざまなモバイルデバイス画面に応答する必要があります。
* IOS をサポートするアプリは、最新バージョンの iOS を使用した最新の iPad デバイスで完全に機能する必要があります。
* アプリ内購入への直接の参照、試用版の提供、有料版の提供、または、ユーザーがモバイル OS (Android、iOS) で Teams アプリから他のコンテンツ、アプリ、またはアドインを購入または取得できる任意のオンラインストアへのリンクを含めることはできません。
...
Teams アプリは、そのデータを表示することを意図していない対象ユーザーに、クレジットカードや金融の支払い機器、個人を特定できる情報 (PIN)、健康、連絡先のトレース情報などの機密データを投稿してはなりません。
...
### <a name="9989-clear-warning-before-downloading-any-files-or-executable-exe-into-a-users-environment"></a>ユーザーの環境にファイルまたは実行可能ファイル () をダウンロードする前に警告を &#9989; をクリアする `.exe`

アプリ `.exe`  でユーザーのコンピューターまたは環境に任意のファイルまたは実行可能ファイル () をダウンロードする前に、ユーザーに警告してください。

### <a name="9989-messaging-extensions-should-provide-help-text-and-be-easy-to-read"></a>&#9989; メッセージング拡張機能は、ヘルプテキストを提供し、読みやすくする必要があります。

* 検索ベースのメッセージング拡張機能は、効果的な検索方法 (例: 入力例を表示するなど) に関するヘルプテキストを提供する必要があります。
* タスクモジュールには、アイコンと、アプリに含まれるか作成された短い名前を含める必要があります。
* メッセージ拡張機能の実行可能ファイルは、明確で理解しやすいものにし `@mention` て、読みやすくする必要があります。
![メッセージの内線番号](../../../../assets/images/faq/message-extension.png)
