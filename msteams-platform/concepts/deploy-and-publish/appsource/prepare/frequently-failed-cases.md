...
---
# <a name="tips-for-a-successful-microsoft-teams-app-submission"></a>Microsoft Teams アプリの申請に成功するためのヒント

この記事では、送信されたアプリが検証に失敗する一般的な理由について説明します。 アプリに関して発生する可能性のあるすべての問題の完全な一覧を示す目的ではありませんが、このガイドに従って、アプリの申請が初めて渡される可能性が高くなることができます。 検証 [ポリシーの広範なリストについては、「](/legal/marketplace/certification-policies) 商用マーケットプレース認定ポリシー」を参照してください。

>[!NOTE]
>**[セクション 1140 は](/legal/marketplace/certification-policies#1140-teams)** Microsoft Teams に固有で、セクション **[1140.4](https://docs.microsoft.com/legal/marketplace/certification-policies#11404-functionality)** では Teams アプリの機能要件に対応しています。

## <a name="validation-guidelines--most-failed-test-cases"></a>ほとんどのテスト ケース&検証ガイドライン

### <a name="9989-general-considerations"></a>&#9989;一般的な考慮事項

「セクション [100 — 全般」も参照してください。](/legal/marketplace/certification-policies#100-general)

* Microsoft Teams SDK のバージョン 1.4.1 以降を使用 [している必要があります](https://www.npmjs.com/package/@microsoft/teams-js)。
...
* ビジュアル ツアーには、ビデオ、スクリーンショット、アプリの詳細を含む静的タブへのリンクを指定できます。 これらの参照はすべて Teams 環境内にある必要があります。
...
* アプリは、主要なユーザー シナリオのために Teams からユーザーを取り出しなけれ。 アプリ内のリンク ターゲットは、外部ブラウザーにリンクしなけい。 リンク ターゲットは、Teams に含まれる div 要素 (タスク モジュールやタブなど) にリンクする必要があります。 
* Teams 内のユーザーに情報を表示するには、タスク モジュールまたはタブの使用を推奨します。
* 以下を除き、すべてのコアシナリオとコア以外のシナリオを Teams 環境内で完了する必要があります。
  * プライバシー ポリシー
  * 利用規約 (TOU)
  * [Web サイト] リンク
  * サインアップ プロセス
...
「セクション [1100.5 - 顧客コントロール」も参照してください。](/legal/marketplace/certification-policies#11005-customer-control)
...
* サインアウト オプションは、Teams クライアントからではなく、アプリの機能からユーザーをサインアウトする必要があります。
...
* サインインとサインアウトの機能は、モバイル クライアントで機能する必要があります。 Microsoft Teams [SDK](https://www.npmjs.com/package/@microsoft/teams-js) バージョン 1.4.1 以降を使用してください。
...
* チャネル タブには、メインの Teams ナビゲーションと競合する左側のレールにアイコンが含まれています。
* タブには、メインの Teams ナビゲーションと競合するアイコンが左側のレールに表示されていないアプリ バーを表示する必要があります。
...
* アプリ ヘッダーから *使用* できる [設定] タブを含める必要があります。
...
* 構成エクスペリエンスは Teams コンテキスト内に残る必要があります。 ユーザーは、コンテンツを作成するために構成エクスペリエンスを離れ、Teams に戻ってピン留めする必要があります。
...
[タスク モジュールを呼び出すボタンを備え、アダプティブ カードを使用すると、](/task-modules-and-cards/task-modules/task-modules-bots)ボットのユーザー エクスペリエンスが向上します。 これらのカードとボタンは、ユーザーがコマンドを入力するのとは対照的に、モバイル デバイスで使用する方が簡単です。 また、ボットの応答は、長いテキストを含むテキストである必要があります。 ボットは、会話型チャット ベースのユーザー インターフェイスと長いテキスト応答の代わりに、アダプティブ カードとタスク モジュールを使用する必要があります。

* **すべてのスコープについて考える**. チャネルと個人の会話で言及 ( ) を行う場合は、ボットが適切な `@*botname*` 応答を提供します。 ボットが個人用スコープまたはチーム スコープ内で意味のあるコンテキストを提供しない場合は、マニフェストを使用してそのスコープを無効にします。 (Microsoft `bots` Teams マニフェスト スキーマリファレンスのブロック [を参照してください](../../../../resources/schema/manifest-schema.md#bots)。
...
* iOS をサポートするアプリは、最新バージョンの iOS を使用して最新の iPad デバイスで完全に機能している必要があります。
* モバイル OS (Android、iOS) で Teams アプリから他のコンテンツ、アプリ、アドインを購入または取得できるオンライン ストアへのアプリ内購入、試用版、有料版のオファー、またはリンクへの直接参照は含めずにしてください。
...
Teams アプリは、クレジット カードや金融支払い手段、個人識別可能な情報 (PIN)、正常性、連絡先トレース情報などの機密データを、そのデータを表示することを意図していない対象ユーザーに投稿しなけってはなからねない。

### <a name="9989-do-not-transmit-financial-payment-details-or-complete-financial-transactions-via-your-teams-app"></a>&#9989; Teams アプリを介して金銭的な支払いの詳細を送信したり、金融取引を完了したりしない

* Teams アプリは、Teams インターフェイス内で直接支払いを行うユーザーに頼む必要があります。
...
### <a name="9989-complete-the-publisher-attestation-in-partner-center"></a>&#9989; パートナー センターで発行元の構成証明を完了する

* 詳細については、「 [完全な Publisher 構成証明プログラム](/microsoft-365-app-certification/docs/attestation) 」のドキュメントを参照してください。
* [発行元構成証明ワークフロー] セクションの手順に [従](/microsoft-365-app-certification/docs/userguide#3publisher-attestation-workflow) って、発行元の構成証明プロセスを完了します。 質問については、appcert@microsoft.com に書き込みしてください。
