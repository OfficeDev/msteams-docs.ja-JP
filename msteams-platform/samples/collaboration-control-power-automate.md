...
author: surbhigupta
description: このモジュールでは、Microsoft Teamsのコラボレーション コントロール アプリの Power Automate と Azure アプリの作成方法について説明します。
...
1. Dynamics CRM を選択した後、アクセス許可で **user_impersonation** を選択してください。
   ...
   :::image type="content" source="../assets/images/collaboration-control/admin-consent-required.png" alt-text="スクリーンショットは、チェックボックスを有効にする方法を示す例user_impersonation":::
...
1. Power Apps ポータルを開き、右上隅で **設定** を選択し、**中央管理開きます**。
   ...
   :::image type="content" source="../assets/images/collaboration-control/power-apps-interface.png" alt-text="スクリーンショットは、Power Apps インターフェイスを示す例です":::
...
1. [設定の詳細] ページで、[ **ユーザーとアクセス許可** ] セクションを選択し、[ **アプリケーション ユーザー**] を選択します。
   ...
   :::image type="content" source="../assets/images/collaboration-control/users-link.png" alt-text="スクリーンショットは、アプリケーションユーザーのリンクを示す例です":::
...
1. PowerApps または Power Automate を開き、 **カスタム コネクタ メニューを** 選択します。 コラボレーション コネクタの **編集** を選択します。
   ...
   :::image type="content" source="../assets/images/collaboration-control/collaboration-connector.png" alt-text="カスタム コネクタ メニュー":::

1. [全般情報] タブで、Dynamic 365 インスタンス ドメイン (https:// なし) のアドレスを持つホストを入力します。

   :::image type="content" source="../assets/images/collaboration-control/general-information.png" alt-text="スクリーンショットは、一般的な情報を示す例です":::
...
* **トリガー**
  ...
  1. コラボレーション セッションが作成されたとき。
      ...
      :::image type="content" source="../assets/images/collaboration-control/colab-session-created-preview.png" alt-text="作成されたコラボレーション セッション":::
  ...
  1. タスクが作成または変更されたとき
      ...
      既定では、Planner タスクのトリガーは無効になり、トリガーされません。 これを有効にするには、テナント管理者が次の手順を完了する必要があります。

      * Power Apps/Collaboration コントロール/設定のパスの下にサポート チケットを作成します。
      * コラボレーション コネクタに対して環境を有効にし、環境 URL (推奨) または組織 ID を提供することを要求します。  
      * サポート要求に次のサンプル テキストを追加できます。"環境 URL を有効にする: `url` コラボレーション コネクタ用" です。
      * サポート チケットを開くには、「[ヘルプとサポートを受ける](/power-platform/admin/get-help-support)」を参照してください
...
* **アクション**
  ...
  1. コラボレーション セッションの開始
      ...
      **メタデータ (詳細):** コラボレーション セッションのメタデータを追加します。
        ...
        * **鍵：** メタデータ属性に関連付けられているキー。
        * **価値：** メタデータ属性に関連付けられている値。
  ...
  1. コラボレーション セッションの更新
     ...
     ***詳細オプション:***
      ...
      * **OData 型:** 他のキー/値が設定され、#Microsoft.Dynamics.CRM.m365_collaborationmetadata と正確に一致する必要がある場合は、このフィールドを指定する必要があります。
      * **鍵：** メタデータ属性に関連付けられているキー。
      * **価値：** メタデータ属性に関連付けられている値。
      ...
      * **OData 型:** 他のキー/値が設定され、#Microsoft.Dynamics.CRM.m365_collaborationmetadata と正確に一致する必要がある場合は、このフィールドを指定する必要があります。
      * **鍵：** 更新するメタデータ属性に関連付けられているキー。
      * **価値：** メタデータ属性に関連付けられている値。

      **メタデータの削除 (詳細):** コラボレーション セッション レコードの既存のメタデータをすべて削除します。

      * **OData 型:** 他のキー/値が設定され、#Microsoft.Dynamics.CRM.m365_collaborationmetadata と正確に一致する必要がある場合は、このフィールドを指定する必要があります。
      * **鍵：** 削除するメタデータ属性に関連付けられているキー。
  ...
  1. Planner タスクの作成
      ...
      :::image type="content" source="../assets/images/collaboration-control/create-planner-task.png" alt-text="スクリーンショットは、Planner タスクを作成する方法を示す例です。":::
     ...
     * **優先権：** タスクの優先度。 0 と 10 (包括) の増加値は優先順位が低くなります。
  ...
  1. Planner タスクの更新
      ...
      :::image type="content" source="../assets/images/collaboration-control/update-planner-task-preview.png" alt-text="プランナー タスクを更新する":::

     このステップ アクションは、コラボレーション コントロール Planner タスク仮想テーブルを使用して、Planner タスク レコードを更新します。

     * **タスク ID (必須):** タスクの一意の識別子。
     * **割り当て：** Task のすべての割り当てを表す json 形式のオブジェクト。 見る。 plannerAssignments リソースの種類 - Microsoft Graph v1.0 |Microsoft Docs  
     * **バケット ID:** タスクが属するバケット ID。  
...
1. コラボレーション セッションが作成されるたびに、詳細をキャプチャして電子メール通知を送信します。
   ...
   :::image type="content" source="../assets/images/collaboration-control/colab-session-created-preview.png" alt-text="スクリーンショットは、作成されたコラボレーション セッションを示す例です":::
