...
複数のチャンネルまたはグループタブを設定でき、アプリごとに最大 16 個の個人用タブを使用できます。

## <a name="mobile-considerations"></a>モバイルに関する考慮事項

モバイル クライアントにチャンネルタブまたはグループ タブを表示Teams選択した場合、 `setSettings()` 設定にはプロパティの値が必要 `websiteUrl` です。 ユーザー エクスペリエンスを最適化するには、タブを作成するときに [モバイルのタブのガイダンスに](~/tabs/design/tabs-mobile.md) 従う必要があります。 [Teams ストアを通じて配布される](~/concepts/deploy-and-publish/appsource/publish.md)アプリには、モバイル クライアント用に個別の承認プロセスがあります。 このようなアプリの既定の動作は次のとおりです。

| **アプリの機能** | **アプリが承認された場合の動作** | **アプリが承認されない場合の動作** |
| --- | --- | --- |
| **個人用タブ** | モバイル クライアントの下部バーにアプリが表示されます。 Teamsクライアントでタブが開きます。 | モバイル クライアントの下部バーにアプリが表示されません。 |
| **チャンネルタブとグループタブ** | タブは、Teams クライアントで を使用して開きます `contentUrl` 。 | タブは、 を使用して Teams クライアントの外部のブラウザーで開きます `websiteUrl` 。 |

> [!NOTE]
>
> アプリの既定の動作は、Teams ストアを通じて配布される場合にのみ適用されます。 既定では、すべてのタブがTeams クライアントで開きます。
> モバイルフレンドリーに対するアプリの評価を開始するには、アプリの詳細に teamsubm@microsoft.com に連絡します。
