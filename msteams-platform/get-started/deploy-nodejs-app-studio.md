...
description: App Studio でアプリを使用Microsoft TeamsアプリNode.js展開する方法について
...
1. アプリのアプリ パッケージを作成するにはTeams App Studio の **[マニフェスト エディター]** タブ **を選択します**。
    ...
    このサンプルには独自のマニフェストが付属し、プロジェクトのビルド時にアプリ パッケージをビルドするように設計されています。 このNode.js、プロジェクトの `gulp` ルート ディレクトリのコマンド ラインに入力します。

    プロジェクトのルート ディレクトリ `gulp` のコマンド Node.js入力して、アプリ パッケージをビルドできます。
...
ngrok を使用してアプリを実行している場合は、ローカル環境変数を設定する必要があります。 次のコマンドを使用Visual Studio Code起動構成[を追加できます](https://code.visualstudio.com/Docs/editor/debugging#_launch-configurations)。
...
> [!Note]
> チュートリアルの前のバージョンから npm `npm stop` を停止していない場合は、起動構成変数を正しく取得Visual Studio Code実行する必要があります。
...
1. 次に、[タブの追加`Hello World`**] リストから選択** できます。
1. 構成ダイアログで、チャネルに表示するタブを選択します。 次に、[保存] を **選択します**。

選択したタブが読 `Hello World` み込まれたタブを確認できます。

<img width="430px" alt="Screenshot of configure" src="~/assets/images/samples-hello-world-tab-configure.png"/>

### <a name="test-your-bot-in-teams"></a>ボットをテストTeams

これで、ボットと対話できます。Teams。 アプリを登録したチーム内のチャネルを選択し、「」 `@your-bot-name`と入力し、その後にメッセージを入力します。 この種類のメッセージはメンションと呼 **\@ばれる**。 ボットに送信するメッセージは、返信として返されます。
...
1. 会話ビューの入力ボックスの下にある 3 つのドットを選択します。 「Hello World」アプリ **を含むメニュー** が表示されます。
