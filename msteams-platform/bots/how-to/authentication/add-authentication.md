...
<span data-ttu-id="1e9ca-141">リソースグループとサービスプランは厳密には必要ありませんが、作成したリソースを容易に解放することができます。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-141">The resource group and the service plan aren't strictly necessary, but they allow you to conveniently release the resources you create.</span></span> <span data-ttu-id="1e9ca-142">これは、リソースを整理して管理できるようにするための適切な方法です。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-142">This is good practice for keeping your resources organized and manageable.</span></span>
...
## <a name="create-the-service-plan"></a><span data-ttu-id="1e9ca-162">サービスプランを作成する</span><span class="sxs-lookup"><span data-stu-id="1e9ca-162">Create the service plan</span></span>
...
## <a name="create-the-bot-channels-registration"></a><span data-ttu-id="1e9ca-188">Bot チャネル登録を作成する</span><span class="sxs-lookup"><span data-stu-id="1e9ca-188">Create the bot channels registration</span></span>
...
1. <span data-ttu-id="1e9ca-203">右パネルで、左上にある [ **新しい登録** ] タブを選択します。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-203">In the right panel, select the **New registration** tab, in the upper left.</span></span>
...
### <a name="configure-the-identity-provider-connection-and-register-it-with-the-bot"></a><span data-ttu-id="1e9ca-228">Id プロバイダー接続を構成し、bot に登録する</span><span class="sxs-lookup"><span data-stu-id="1e9ca-228">Configure the identity provider connection and register it with the bot</span></span>
...
1. <span data-ttu-id="1e9ca-238">フォームに次のように入力します。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-238">Complete the form as follows:</span></span>
    ...
    1. <span data-ttu-id="1e9ca-251">**ログイン URL** 。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-251">**Login URL**.</span></span> <span data-ttu-id="1e9ca-252">Enter キーを押し `https://login.microsoftonline.com` ます。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-252">Enter `https://login.microsoftonline.com`.</span></span>
...
1. <span data-ttu-id="1e9ca-292">**[保存]** を選択します。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-292">Select **Save**.</span></span>
...
1. <span data-ttu-id="1e9ca-310">複製の [cs-auth-サンプル][teams-auth-bot-cs]。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-310">Clone [cs-auth-sample][teams-auth-bot-cs].</span></span>
...
1. <span data-ttu-id="1e9ca-326">次のようにして、 **env** 構成を更新します。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-326">Update the **.env** configuration as follows:</span></span>
    ...
    - <span data-ttu-id="1e9ca-327">`MicrosoftAppId`Bot チャネル登録時に保存した **BOT アプリ ID** に設定します。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-327">Set `MicrosoftAppId` to the **bot App ID** you saved at the time of the bot channel registration.</span></span>
...
<span data-ttu-id="1e9ca-341">Bot を展開するには、「 [Azure に bot を展開](https://aka.ms/azure-bot-deployment-cli)する方法」の手順に従います。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-341">To deploy the bot, follow the steps in the how to [Deploy your bot to Azure](https://aka.ms/azure-bot-deployment-cli).</span></span>
...
## <a name="test-the-bot-using-the-emulator"></a><span data-ttu-id="1e9ca-365">エミュレーターを使用して bot をテストする</span><span class="sxs-lookup"><span data-stu-id="1e9ca-365">Test the bot using the Emulator</span></span>
...
### <a name="test-the-bot-locally"></a><span data-ttu-id="1e9ca-382">Bot をローカルでテストする</span><span class="sxs-lookup"><span data-stu-id="1e9ca-382">Test the bot locally</span></span>
...
1. <span data-ttu-id="1e9ca-398">エミュレーターで使用した構成に応じて、次のいずれかを取得します。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-398">Depending which configuration you used for the Emulator, you get one of the following:</span></span>
    ...
    <span data-ttu-id="1e9ca-404">次の図は、ログインした後の bot UI の例です。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-404">The following image is an example of the bot UI after you've logged in:</span></span>
...
1. <span data-ttu-id="1e9ca-423">要求された場合は、該当するユーザーのアカウントを選択します。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-423">If asked, select the applicable user's account.</span></span>
    ...
    ![auth bot ログインの展開](../../../assets/images/authentication/auth-bot-login-deployed.PNG)<span data-ttu-id="1e9ca-426">.</span><span class="sxs-lookup"><span data-stu-id="1e9ca-426">.</span></span>
...
<span data-ttu-id="1e9ca-460">Microsoft Teams アプリをローカルで実行する準備として ngrok をセットアップするには、次の手順を実行します。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-460">To set up ngrok in preparation for running your Microsoft Teams app locally, follow these steps:</span></span>

1. <span data-ttu-id="1e9ca-461">ターミナルウィンドウで、がインストールされているディレクトリに移動し `ngrok.exe` ます。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-461">In a terminal window, go the directory where you have `ngrok.exe` installed.</span></span> <span data-ttu-id="1e9ca-462">これを指すように、 *環境変数* のパスを設定することをお勧めします。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-462">We suggest setting the *environment variable* path to point to it.</span></span>
1. <span data-ttu-id="1e9ca-463">たとえば、を実行 `ngrok http 3978 --host-header=localhost:3978` します。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-463">Run, for example, `ngrok http 3978 --host-header=localhost:3978`.</span></span> <span data-ttu-id="1e9ca-464">必要に応じてポート番号を置き換えます。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-464">Replace the port number as needed.</span></span>
<span data-ttu-id="1e9ca-465">これにより、指定したポートをリッスンする ngrok が起動します。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-465">This launches ngrok to listen on the port you specify.</span></span> <span data-ttu-id="1e9ca-466">これにより、ngrok が実行されている限り、外部的にアドレス指定可能な URL が得られます。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-466">In return, it gives you an externally addressable URL, valid for as long as ngrok is running.</span></span> <span data-ttu-id="1e9ca-467">次の画像は例です。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-467">The following image is an example:</span></span>

    ![teams bot アプリ認証の接続文字列 adv1](../../../assets/images/authentication/auth-bot-ngrok-start.PNG)<span data-ttu-id="1e9ca-469">.</span><span class="sxs-lookup"><span data-stu-id="1e9ca-469">.</span></span>

1. <span data-ttu-id="1e9ca-470">転送用の HTTPS アドレスをコピーします。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-470">Copy the forwarding HTTPS address.</span></span> <span data-ttu-id="1e9ca-471">これは、次のようになり `https://dea822bf.ngrok.io/` ます。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-471">It should be similar to the following: `https://dea822bf.ngrok.io/`.</span></span>
1. <span data-ttu-id="1e9ca-472">`/api/messages`取得する追加 `https://dea822bf.ngrok.io/api/messages` 。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-472">Append `/api/messages` to obtain `https://dea822bf.ngrok.io/api/messages`.</span></span> <span data-ttu-id="1e9ca-473">これは、コンピューター上でローカルに実行されていて、Microsoft Teams のチャットで web 経由でアクセスできる bot の **メッセージエンドポイント** です。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-473">This is the **messages endpoint** for the bot running locally on your machine and reachable over the web in a chat in Microsoft Teams.</span></span>
1. <span data-ttu-id="1e9ca-474">最後に実行する手順の1つは、展開した bot のメッセージエンドポイントを更新することです。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-474">One final step to perform is to update the messages endpoint of the deployed bot.</span></span> <span data-ttu-id="1e9ca-475">この例では、bot を Azure に展開しました。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-475">In the example, we deployed the bot in Azure.</span></span> <span data-ttu-id="1e9ca-476">そのため、以下の手順を実行してみましょう。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-476">So \*\*let's perform these steps:</span></span>
    1. <span data-ttu-id="1e9ca-477">ブラウザーで [**Azure ポータル**][azure-portal]に移動します。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-477">In your browser navigate to the [**Azure portal**][azure-portal].</span></span>
    1. <span data-ttu-id="1e9ca-478">**Bot チャネル登録** を選択します。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-478">Select your **Bot Channel Registration**.</span></span>
    1. <span data-ttu-id="1e9ca-479">左側のパネルで、[ **設定** ] を選択します。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-479">In the left panel, select **Settings**.</span></span>
    1. <span data-ttu-id="1e9ca-480">右側のパネルの [ **メッセージングエンドポイント** ] ボックスに、ngrok URL (この例では) を入力し `https://dea822bf.ngrok.io/api/messages` ます。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-480">In the right panel, in the **Messaging endpoint** box, enter the ngrok URL, in our example, `https://dea822bf.ngrok.io/api/messages`.</span></span>
1. <span data-ttu-id="1e9ca-481">Visual Studio デバッグモードなどで、ボットをローカルに起動します。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-481">Start your bot locally, for example in Visual Studio debug mode.</span></span>
1. <span data-ttu-id="1e9ca-482">Bot フレームワークポータルの **テスト Web チャット** を使用して、ローカルで実行中に bot をテストします。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-482">Test the bot while running locally using the Bot Framework portal's **Test Web chat**.</span></span> <span data-ttu-id="1e9ca-483">エミュレーターと同様に、このテストでは Teams 固有の機能にアクセスすることはできません。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-483">Like the Emulator, this test doesn't allow you to access Teams-specific functionality.</span></span>
1. <span data-ttu-id="1e9ca-484">が実行されているターミナルウィンドウで、 `ngrok` ボットと web チャットクライアント間の HTTP トラフィックを確認できます。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-484">In the terminal window where `ngrok` is running you can see HTTP traffic between the bot and the web chat client.</span></span> <span data-ttu-id="1e9ca-485">より詳細な表示が必要な場合は、ブラウザーウィンドウで `http://127.0.0.1:4040` 以前のターミナルウィンドウから取得した情報を入力します。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-485">If you want a more detailed view, in a browser window enter `http://127.0.0.1:4040` you obtained from the previous terminal window.</span></span> <span data-ttu-id="1e9ca-486">次の画像は例です。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-486">The following image is an example:</span></span>

    ![auth bot teams ngrok テスト](../../../assets/images/authentication/auth-bot-teams-ngrok-testing.png)<span data-ttu-id="1e9ca-488">.</span><span class="sxs-lookup"><span data-stu-id="1e9ca-488">.</span></span>

> [!NOTE]
> <span data-ttu-id="1e9ca-489">Ngrok を停止してから再起動すると、URL が変更されます。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-489">If you stop and restart ngrok, the URL changes.</span></span> <span data-ttu-id="1e9ca-490">プロジェクトで ngrok を使用し、使用している機能に応じて、すべての URL 参照を更新する必要があります。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-490">To use ngrok in your project, and depending on the capabilities you're using, you must update all URL references.</span></span>
 

## <a name="additional-information"></a><span data-ttu-id="1e9ca-491">追加情報</span><span class="sxs-lookup"><span data-stu-id="1e9ca-491">Additional information</span></span>

### <a name="teamsappmanifestmanifestjson"></a><span data-ttu-id="1e9ca-492">TeamsAppManifest/manifest.js</span><span class="sxs-lookup"><span data-stu-id="1e9ca-492">TeamsAppManifest/manifest.json</span></span>

<span data-ttu-id="1e9ca-493">このマニフェストには、Microsoft Teams が bot と接続するために必要な情報が含まれています。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-493">This manifest contains information needed by Microsoft Teams to connect with the bot.</span></span>  
...
<span data-ttu-id="1e9ca-494">認証を使用すると、以下に説明するように、Teams は他のチャネルとはやや異なる方法で動作します。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-494">With authentication, Teams behaves slightly differently than other channels, as explained below.</span></span>

### <a name="handling-invoke-activity"></a><span data-ttu-id="1e9ca-495">呼び出しアクティビティの処理</span><span class="sxs-lookup"><span data-stu-id="1e9ca-495">Handling Invoke Activity</span></span>

<span data-ttu-id="1e9ca-496">**呼び出しアクティビティ** は、他のチャネルによって使用されるイベントアクティビティではなく bot に送信されます。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-496">An **Invoke Activity** is sent to the bot rather than the Event Activity used by other channels.</span></span>
<span data-ttu-id="1e9ca-497">これは、 **Activityhandler** のサブクラスによって実行されます。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-497">This is done by sub-classing the **ActivityHandler**.</span></span>

# <a name="cnet"></a>[<span data-ttu-id="1e9ca-498">C#/.NET</span><span class="sxs-lookup"><span data-stu-id="1e9ca-498">C#/.NET</span></span>](#tab/dotnet-sample)

<span data-ttu-id="1e9ca-499">**ボット/の bot**</span><span class="sxs-lookup"><span data-stu-id="1e9ca-499">**Bots/DialogBot.cs**</span></span>

[!code-csharp[ActivityHandler](~/../botbuilder-samples/samples/csharp_dotnetcore/46.teams-auth/Bots/DialogBot.cs?range=19-51)]

<span data-ttu-id="1e9ca-500">**ボット/TeamsBot**</span><span class="sxs-lookup"><span data-stu-id="1e9ca-500">**Bots/TeamsBot.cs**</span></span>

<span data-ttu-id="1e9ca-501">**Oauthprompt** を使用している場合は、ダイアログに *呼び出しアクティビティ* を転送する必要があります。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-501">The *Invoke Activity* must be forwarded to the dialog if the **OAuthPrompt** is used.</span></span>

[!code-csharp[ActivityHandler](~/../botbuilder-samples/samples/csharp_dotnetcore/46.teams-auth/Bots/TeamsBot.cs?range=34-42)]

#### <a name="teamsactivityhandlercs"></a><span data-ttu-id="1e9ca-502">TeamsActivityHandler.cs</span><span class="sxs-lookup"><span data-stu-id="1e9ca-502">TeamsActivityHandler.cs</span></span>
...
# <a name="javascript"></a>[<span data-ttu-id="1e9ca-503">JavaScript</span><span class="sxs-lookup"><span data-stu-id="1e9ca-503">JavaScript</span></span>](#tab/node-js-dialog-sample)

<span data-ttu-id="1e9ca-504">**bot/dialogBot.js**</span><span class="sxs-lookup"><span data-stu-id="1e9ca-504">**bots/dialogBot.js**</span></span>

[!code-javascript[ActivityHandler](~/../botbuilder-samples/samples/javascript_nodejs/46.teams-auth/bots/dialogBot.js?range=4-46)]

<span data-ttu-id="1e9ca-505">**bot/teamsBot.js**</span><span class="sxs-lookup"><span data-stu-id="1e9ca-505">**bots/teamsBot.js**</span></span>

<span data-ttu-id="1e9ca-506">**Oauthprompt** を使用している場合は、ダイアログに *呼び出しアクティビティ* を転送する必要があります。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-506">The *Invoke Activity* must be forwarded to the dialog if the **OAuthPrompt** is used.</span></span>

[!code-javascript[ActivityHandler](~/../botbuilder-samples/samples/javascript_nodejs/46.teams-auth/bots/teamsBot.js?range=4-33)]

<span data-ttu-id="1e9ca-507">**ダイアログ/mainDialog.js**</span><span class="sxs-lookup"><span data-stu-id="1e9ca-507">**dialogs/mainDialog.js**</span></span>

<span data-ttu-id="1e9ca-508">ダイアログステップで、を使用し `beginDialog` て OAuth プロンプトを開始します。これにより、ユーザーにサインインするように求められます。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-508">Within a dialog step, use `beginDialog` to start the OAuth prompt, which asks the user to sign in.</span></span>

- <span data-ttu-id="1e9ca-509">ユーザーが既にサインインしている場合は、ユーザーに確認を求めることなく、トークン応答イベントが生成されます。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-509">If the user is already signed in, this will generate a token response event, without prompting the user.</span></span>
- <span data-ttu-id="1e9ca-510">それ以外の場合は、ユーザーにサインインを求めるメッセージが表示されます。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-510">Otherwise, this will prompt the user to sign in.</span></span> <span data-ttu-id="1e9ca-511">Azure Bot サービスは、ユーザーがサインインしようとした後に、トークン応答イベントを送信します。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-511">The Azure Bot Service sends the token response event after the user attempts to sign in.</span></span>

[!code-javascript[AddOAuthPrompt](~/../botbuilder-samples/samples/javascript_nodejs/46.teams-auth/dialogs/mainDialog.js?range=50-52)]

<span data-ttu-id="1e9ca-512">次のダイアログステップでは、前の手順の結果にトークンが存在するかどうかを確認します。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-512">Within the following dialog step, check for the presence of a token in the result from the previous step.</span></span> <span data-ttu-id="1e9ca-513">Null でない場合、ユーザーは正常にサインインしています。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-513">If it is not null, the user successfully signed in.</span></span>

[!code-javascript[AddOAuthPrompt](~/../botbuilder-samples/samples/javascript_nodejs/46.teams-auth/dialogs/mainDialog.js?range=50-64)]

<span data-ttu-id="1e9ca-514">**bot/logoutDialog.js**</span><span class="sxs-lookup"><span data-stu-id="1e9ca-514">**bots/logoutDialog.js**</span></span>

[!code-javascript[allow-logout](~/../botbuilder-samples/samples/javascript_nodejs/46.teams-auth/dialogs/logoutDialog.js?range=31-42&highlight=7)]

# <a name="python"></a>[<span data-ttu-id="1e9ca-515">Python</span><span class="sxs-lookup"><span data-stu-id="1e9ca-515">Python</span></span>](#tab/python-sample)

<span data-ttu-id="1e9ca-516">**ボット/dialog_bot py**</span><span class="sxs-lookup"><span data-stu-id="1e9ca-516">**bots/dialog_bot.py**</span></span>

[!code-python[ActivityHandler](~/../botbuilder-samples/samples/python/46.teams-auth/bots/dialog_bot.py?range=10-42)]

<span data-ttu-id="1e9ca-517">**ボット/teams_bot py**</span><span class="sxs-lookup"><span data-stu-id="1e9ca-517">**bots/teams_bot.py**</span></span>

<span data-ttu-id="1e9ca-518">**Oauthprompt** を使用している場合は、ダイアログに *呼び出しアクティビティ* を転送する必要があります。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-518">The *Invoke Activity* must be forwarded to the dialog if the **OAuthPrompt** is used.</span></span>

[!code-python[on_token_response_event](~/../botbuilder-samples/samples/python/46.teams-auth/bots/teams_bot.py?range=38-45)]

<span data-ttu-id="1e9ca-519">**ダイアログ/main_dialog py**</span><span class="sxs-lookup"><span data-stu-id="1e9ca-519">**dialogs/main_dialog.py**</span></span>

<span data-ttu-id="1e9ca-520">ダイアログステップで、を使用し `begin_dialog` て OAuth プロンプトを開始します。これにより、ユーザーにサインインするように求められます。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-520">Within a dialog step, use `begin_dialog` to start the OAuth prompt, which asks the user to sign in.</span></span>

- <span data-ttu-id="1e9ca-521">ユーザーが既にサインインしている場合は、ユーザーに確認を求めることなく、トークン応答イベントが生成されます。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-521">If the user is already signed in, this will generate a token response event, without prompting the user.</span></span>
- <span data-ttu-id="1e9ca-522">それ以外の場合は、ユーザーにサインインを求めるメッセージが表示されます。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-522">Otherwise, this will prompt the user to sign in.</span></span> <span data-ttu-id="1e9ca-523">Azure Bot サービスは、ユーザーがサインインしようとした後に、トークン応答イベントを送信します。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-523">The Azure Bot Service sends the token response event after the user attempts to sign in.</span></span>

[!code-python[Add OAuthPrompt](~/../botbuilder-samples/samples/python/46.teams-auth/dialogs/main_dialog.py?range=48-49)]

<span data-ttu-id="1e9ca-524">次のダイアログステップでは、前の手順の結果にトークンが存在するかどうかを確認します。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-524">Within the following dialog step, check for the presence of a token in the result from the previous step.</span></span> <span data-ttu-id="1e9ca-525">Null でない場合、ユーザーは正常にサインインしています。</span><span class="sxs-lookup"><span data-stu-id="1e9ca-525">If it is not null, the user successfully signed in.</span></span>

[!code-python[Add OAuthPrompt](~/../botbuilder-samples/samples/python/46.teams-auth/dialogs/main_dialog.py?range=51-61)]

<span data-ttu-id="1e9ca-526">**ダイアログ/logout_dialog py**</span><span class="sxs-lookup"><span data-stu-id="1e9ca-526">**dialogs/logout_dialog.py**</span></span>
...
> [<span data-ttu-id="1e9ca-527">Azure Bot サービスによる認証の追加について</span><span class="sxs-lookup"><span data-stu-id="1e9ca-527">Learn about adding authentication via Azure Bot Service</span></span>](https://aka.ms/azure-bot-add-authentication)
